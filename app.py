...
def process_audio(farsi_audio_path):
    ...
    try:
        ...
        segments = transcribe_and_translate(farsi_audio_path)
        ...
        # Use a unique filename to prevent race conditions among multiple users
        output_file = f"english_dubbed_{uuid.uuid4().hex[:8]}.mp3"

        final_audio_path = synchronize_audio(segments, output_file)
